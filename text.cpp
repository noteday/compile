...
int main()
{
    ...
    printf("------Input Data：\n");
    sprintf(output,"------Input Data------\n");
    fprintf(fpout, output);
    FILE *fp=NULL;
    ...
    while((input[p_input]=getc(fp))!='#')
    {
        putchar(input[p_input]);
        sprintf(output,"%c",input[p_input]);
        fprintf(fpout, output);
        p_input++;
    }
    ...
    sprintf(output,"There are %d lines,and there are %d errors\n",row,error_row);
    fprintf(fpout, output);
    return 0;
}
...
int Irparser()
{
    ...
    if(oneword->typenum == 1)//begin单词的种别码就是1，该程序中是从begin开始的
    {
        ...
        {
                printf("第%d行：缺少‘end’\n",row);
                sprintf(output,"line %d :'end' limited\n",row);
                fprintf(fpout, output);
                error_row++;
                kk=1;
        }
    ...
    {
        printf("开始处：缺少‘begin’\n");
        sprintf(output,"at the begining :'begin' limited\n",row);
        fprintf(fpout, output);
...
{
      printf("(%d,%s,%s)\n",oneword->typenum,oneword->word,oneword->word);
    ...
    {
        printf("第%d行：变量名错误\n",row);
        sprintf(output,"line %d :name error\n",row);
        fprintf(fpout, output);
        error_row++;//出错个数加1
        kk=1;
        error_enter();
...
int factor()//因子的判断
{
    ...
    {
        printf("第%d行：表达式错误\n",row);
        sprintf(output,"line %d :the expression is wrong\n",row);
        fprintf(fpout, output);
        error_row++;//出错格式加1
        kk=1;//标志出错
